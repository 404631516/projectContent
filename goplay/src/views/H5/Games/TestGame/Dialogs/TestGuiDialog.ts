...
import { EnvironmentalAdditionType, TestNumber, TestString } from '../Data/TestConfig';
...
export default class TestGuiDialog extends UIDialog {
  /** 遊戲場景 */
  public declare scene: TestGameScene;
  ...
  private setMainHitZone(): void {
    const mainHitZone = this.addZone(
      this.centerX,
      // 海平面之下
      (this.height - TestNumber.SeaLevel) / 2 + TestNumber.SeaLevel,
      this.width,
      // 與海水高度相同
      this.height - TestNumber.SeaLevel
    );
  ...
  private setHintComponent(): void {
    ...
    this.addObject(
      ...
      HintLayout,
      TestString.IconEnvironmentalAddition,

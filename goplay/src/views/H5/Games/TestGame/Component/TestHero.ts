...
import { TestString, TestDepth } from '../Data/TestConfig';
import TestHeroFSM, { TestHeroEventId, TestHeroStateId } from './TestHeroFSM';
...
// Hero prefab
export default class TestHero extends Object2D {
  //#region readonly
  /** 尺寸 */
  ...
  public get isMove(): boolean {
    return this.heroFSM.isCurrentState(TestHeroStateId.Move);
  }

  /** 現在是否為發射魚竿狀態 */
  public get isCast(): boolean {
    return this.heroFSM.isCurrentState(TestHeroStateId.Cast);
  }

  /** 現在是否為拉起魚竿狀態 */
  public get isPullUp(): boolean {
    return this.heroFSM.isCurrentState(TestHeroStateId.PullUp);
  }

  /** 現在是否為發射魚網狀態 */
  public get isCastNet(): boolean {
    return this.heroFSM.isCurrentState(TestHeroStateId.CastNet);
  }

  /** 現在是否為拉起魚網狀態 */
  public get isPullUpNet(): boolean {
    return this.heroFSM.isCurrentState(TestHeroStateId.PullUpNet);
  }
  //#endregion
  ...
  constructor(scene: Scene, position: Phaser.Math.Vector2, heroData: HeroData, settingData: FishingSettingData) {
    ...
    // 設定深度
    this.setDepth(TestDepth.Hero);

    // 初始化
    this.setHero(heroData);
    this.setBoat(TestString.Boat);
    this.setfishingLine(TestString.String);
    this.setfishingNet(TestString.FishingNet);
    this.setFxCompnent();
    ...
    // 設置音效
    this.hurtSoundPool = new SoundPool(this.scene, TestString.AudioHurt);
    this.castSoundPool = new SoundPool(this.scene, TestString.AudioCast);
    this.fishingNetSoundPool = new SoundPool(this.scene, TestString.AudioFishingNet);
    this.invisibleSoundPool = new SoundPool(this.scene, TestString.AudioInvisible);
    this.speedUpSoundPool = new SoundPool(this.scene, TestString.AudioSpeedUp);

    // 啟動狀態機
    this.heroFSM = new TestHeroFSM(this);
    this.heroFSM.start();
  ...
  /** 英雄移動 */
  public move(): void {
    this.heroFSM.stateMachine.triggerEvent(TestHeroEventId.Move);
  }
  /** 英雄發射魚鉤 */
  public cast(): void {
    this.heroFSM.stateMachine.triggerEvent(TestHeroEventId.Cast);
  }
  /** 英雄發射魚網 */
  public castNet(item: TestItem): void {
    this.maxfishingNetScale = item.itemData.multiply;
    this.heroFSM.stateMachine.triggerEvent(TestHeroEventId.CastNet);
  }
  /** 英雄拉起魚鉤 */
  public pullUp(): void {
    this.heroFSM.stateMachine.triggerEvent(TestHeroEventId.PullUp);
  }
  /** 英雄死亡 */
  public dead(): void {
    this.heroFSM.stateMachine.triggerEvent(TestHeroEventId.Dead);
  }
  ...
  public async onInvincible(itemData: FishingItemData): Promise<void> {
    ...
    // 播放特效
    this.invincibleFx.setVisible(true).play(TestString.AnimInvincible);
    // 標記無敵
  ...
  public onHurt(): void {
    ...
    // 播放受傷特效
    this.hurtFx?.play(TestString.AnimHurt);
  }
  //#endregion
  ...
  private setFxCompnent(): void {
    // 創建加速特效
    this.speedUpFx = this.addSprite(TestString.SpeedUpFx, this.speedUpFxPosition.x, this.speedUpFxPosition.y);
    this.speedUpFx.setScale(this.speedUpFxScale).setFlipX(true);
    this.speedUpFx.setVisible(false).play(TestString.AnimSpeedUp);

    // 創建身體強化特效
    this.invincibleFx = this.addSprite(
      TestString.InvincibleFx,
    ...
    // 創建受傷特效
    this.hurtFx = this.addSprite(TestString.BoomFX, this.hurtFxPosition.x, this.hurtFxPosition.y);

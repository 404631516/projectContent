...
export default class FishPool {
  ...
  private readonly minMaxSpawnY: MinMax = {
    min: TestNumber.SeaLevel + 25,
    max: TestNumber.SeaLevel + TestNumber.SeaHeight,
  };
  //#endregion
  ...
  constructor(scene: Scene, fishDatas: FishingFishData[]) {
    ...
    // 設置音效
    this.fishJumpSoundPool = new SoundPool(this.scene, TestString.AudioFishJump);
    this.fishDropSoundPool = new SoundPool(this.scene, TestString.AudioFishDrop);
  }

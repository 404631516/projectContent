...
import { EnvironmentalAdditionType, TestFishFx, TestNumber, TestString } from '../Data/TestConfig';
...
export default class Fish extends Object2D {
  ...
  constructor(
    ...
    super(scene, x, y);
    // 設置尺寸並加入物理body
    this.setSize(TestNumber.FishFramePixel, TestNumber.FishFramePixel);
  ...
  public onSwimEnter(): void {
    ...
    // 判斷是否在左側
    const isLeftSide = this.x <= TestNumber.SeaCenter;
    ...
    const amplitudeDirection = this.y > TestNumber.SeaLevel + TestNumber.SeaHeight / 2 ? -1 : 1;
    // 設置弦波動畫
